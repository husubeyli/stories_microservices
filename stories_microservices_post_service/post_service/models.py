...
class User(UserMixin, db.Model):
    ...
    story = db.relationship('Story', backref=db.backref('users', lazy=True))
...
association_story_table = db.Table('story_association', db.Model.metadata,
...
class Recipe(SaveMixin, db.Model):
    ...
    def to_dict(self):
        from .schemas.schmas import RecipeSchema
        return RecipeSchema().dumps(self)

    
class Story(SaveMixin, db.Model):
    # relation's
    category_id = db.Column(db.Integer, db.ForeignKey('category.id'),
                            nullable=False)
    owner_id = db.Column(db.Integer, db.ForeignKey('user.id'),
                            nullable=False)
    tags = db.relationship('Tag', secondary=association_story_table,
                           backref=db.backref('story_tags', lazy=True))
    slug = db.Column(db.String(120), nullable=False)
    ...
    def __init__(self, title, description, category_id, owner_id=1, image='',
                 is_published=True, **kwargs):
        self.slug = slugify(kwargs.get('title', ''))
        self.title = title
        self.image = image
    ...
    def to_dict(self):
        from .schemas.schmas import StorySchema
        return StorySchema().dumps(self)